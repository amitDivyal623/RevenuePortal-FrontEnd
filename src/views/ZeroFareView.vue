<template>
  <AppLayout>
    <div class="page-header">
      <div><h1 class="page-title">Zero Fare For App Control</h1></div>
      <div class="breadcrumb">
        <a href="#" @click.prevent>Home</a> /
        <a href="#" @click.prevent>Revenue Protection Admin</a> /
        <span class="breadcrumb-active">Zero Fare For App Control</span>
      </div>
    </div>
    ...
    <AdminModal v-if="modalOpen" title="Edit Case Type Zero Fare" size="md" mode="edit" @close="closeModal" @save="save">
      <div class="form-group">
        <label class="form-label">Case Type</label>
        <input :value="form.caseTypeDisplay" disabled />
      </div>
      <div class="form-group inline-row">
        <label class="form-label">Enable Zero Fare</label>
        <label class="toggle">
...
onMounted(async () => {
  loading.value = true
  try {
    const data = await apiGet('/revp/cases/zerofair/?page_size=100')
    rows.value = data.data ?? []
  } catch (err) {
    await swal.error(err?.data?.detail ?? 'Failed to load zero fare data.')
  } finally {
    loading.value = false
  }
})
...
async function save() {
  try {
    ...
    await apiPut(`/revp/cases/zerofair/${form.caseTypeId}/`, { enabled })
    const row = rows.value.find(r => r.case_type_id === form.caseTypeId)
    if (row) row.enabled = enabled
    closeModal()
    await swal.success('Zero fare setting saved.')
  } catch (err) {
